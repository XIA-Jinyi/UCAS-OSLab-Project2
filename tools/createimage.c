...
static void create_image(int nfiles, char *files[])
{
    ...
    for (int fidx = 0; fidx < nfiles; ++fidx) {
        ...
        if (taskidx >= 0) {
            ...
            strncpy(taskinfo[taskidx].name, *files, TASK_NAME_LEN);
        }
        ...
        if (strcmp(*files, "bootblock") == 0) {
            write_padding(img, &phyaddr, SECTOR_SIZE * 2);
        }

        if (taskidx >= 0) {
            taskinfo[taskidx].end = phyaddr;
        }
                           ...
                           short tasknum, FILE * img)
{
    ...
    /* write task number to the image */
    fseek(img, 0x1f8, SEEK_SET);
    uint32_t task_num = tasknum;
    fseek(img, 0x1f8, SEEK_SET);
    fwrite(&task_num, sizeof(task_num), 1, img);

    /* write os size (blocks) to the image */
    uint32_t os_size = NBYTES2SEC(nbytes_kernel);
    // fseek(img, 0x1fc, SEEK_SET);
    fwrite(&os_size, sizeof(os_size), 1, img);

    /* write task info to the image */
    // fseek(img, 0x200, SEEK_SET);
    fwrite(taskinfo, sizeof(task_info_t), task_num, img);
}
